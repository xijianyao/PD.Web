...
public class FilesUpload : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        extTable.Add("flash", "swf,flv,mp4");
        ...
        {
            if (dirName == "image") { showError("上传文件大小超过限制。"); }
        }
        ...
        if (dirName == "image")
        {
            String tempPath = context.Server.MapPath("~/temp/" + PD.Common.Other.GetGuid() + fileExt);

            imgFile.SaveAs(tempPath);

            PD.Common.ThumNail.MakeThumNail(tempPath, filePath, 600, 0, "MW");

            File.Delete(tempPath);
        }
        else { imgFile.SaveAs(filePath); }
